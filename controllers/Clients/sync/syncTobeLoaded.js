import { executeStoredProc, saveToMongoDB, updateJobStatuses } from "../../../utils/dbUtils.js";

const procedures = [
  { name: '__cli_TobeLoadedReport_to_JSON', collection: 'ClientTobeLoaded' },
];

export const syncTobeLoadedJobs = async (req, res) => {
    try {
      const results = [];
  
      // Find the TobeLoaded procedure from the procedures array
      const proc = procedures.find((p) => p.name === "__cli_TobeLoadedReport_to_JSON");
      if (!proc) {
        throw new Error("To Be Loaded procedure not found");
      }
       
      // Execute stored procedure with detailed logging
      const data = await executeStoredProc(proc.name);
  
      // Save to MongoDB with detailed logging
      await saveToMongoDB(proc.collection, data, false);
  
      // Update job statuses after sync
      await updateJobStatuses();  
      results.push({
        procedure: proc.name,
        status: "success",
        message: "TobeLoaded data synced successfully",
        recordCount: data ? data.length : 0,
      });
  
      res.status(200).json({
        success: true,
        message: "TobeLoaded data synced successfully",
        recordCount: data ? data.length : 0,
        results,
      });
    } catch (error) {
      console.error("❌ Error syncing TobeLoaded data:", error);
      console.error("Error stack:", error.stack);
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : String(error),
        message: "Failed to sync TobeLoaded data",
      });
    }
  };

